...
const Todos = ({ todos, deleteTodo }) => {
  const todoList = todos.length ? (
    todos.map((todo) => {
      ...
      const userDate = new Date(todo.due);
      const currDate = new Date();

      console.log(currDate.getDay() + "   " + userDate.getDay());

      if (
        currDate.getFullYear() > userDate.getFullYear() ||
        (currDate.getFullYear() === userDate.getFullYear() &&
          currDate.getMonth() > userDate.getMonth()) ||
        (currDate.getFullYear() === userDate.getFullYear() &&
          currDate.getMonth() === userDate.getMonth() &&
          currDate.getDate() > userDate.getDate())
      ) {
        color = "orange";
      }
